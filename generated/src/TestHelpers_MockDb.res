...
@genType
and entities = {
    @as("Benchmark") benchmark: entityStoreOperations<Entities.Benchmark.t>,
    @as("Chat") chat: entityStoreOperations<Entities.Chat.t>,
    @as("Game") game: entityStoreOperations<Entities.Game.t>,
    @as("LlmResponse") llmResponse: entityStoreOperations<Entities.LlmResponse.t>,
    @as("Message") message: entityStoreOperations<Entities.Message.t>,
    @as("Stake") stake: entityStoreOperations<Entities.Stake.t>,
    @as("WinningClaim") winningClaim: entityStoreOperations<Entities.WinningClaim.t>,
  }
...
//ENTITY EXECUTION
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.benchmark},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.chat},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.game},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.llmResponse},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.message},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.stake},
    ~getKey=entity => entity.id,
  )
  mockDb->executeRowsEntity(
    ~inMemoryStore,
    ~getInMemTable=self => {self.winningClaim},
    ~getKey=entity => entity.id,
  )
}
